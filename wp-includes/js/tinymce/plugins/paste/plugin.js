...
var paste = (function () {
  ...
  var toBlockElements = function (text, rootTag, rootAttrs) {
    ...
    return paragraphs.length === 1 ? paragraphs[0] : global$3.map(paragraphs, stitch)js');
  };
  ...
  var filterWordContent = function (editor, content) {
    ...
    content = $_4bi2o9j0jjgwecui.filter(content, [
      ...
      [
        ...
        function (str, spaces) {
          return spaces.length > 0 ? spaces.replace(/./, ' ').slice(Math.floor(spaces.length / 2)).split('')js';
        }
      ]
    ]);
